...
class RedWallpaperImage:
    ...
    def __init__(self, input_array, colorspace='BGR', thumbnail=None, cluster=None):
        """
        ...
        if self.cluster:
            ...
            self._set_composite_lab()
        else:
            self.rgb_cluster = None
            self.lab_cluster = None
            self.composite_lab_cluster = None
        

    @classmethod
    def from_filepath(cls, filepath, colorspace='BGR', thumbnail=None, cluster=None):
        """
    ...
    def _set_composite_lab(self, lab_weight=0.5):
        """
        Sets the Image's composite 'L*a*b' color cluster. This 'composite' is composed
        of a weighted sum of a 'L*a*b'-clustered colors and 'RGB'-clustered colors
        transformed to 'L*a*b'. From tests, this appears to derive the best results
        when used to find the color difference, or Delta-E between images.
        The weighting can be adjusted, default of 50/50 split.
        """
        # Convert the rgb color clusters to L*a*b
        rgb = self.convert_color(self.rgb_cluster, 'RGB', 'LAB')
        # Derive the composite L*a*b cluster
        self.composite_lab_cluster = self.lab_cluster*lab_weight + rgb*(1-lab_weight)
